...
public class IdCard {
	...
	private String[] errMsg = new String[] { "身份证完全正确！", "身份证为空！", "身份证长度不正确！",
	...
	public int isCorrect() {
		if (this.isEmpty()) {
			this.error = IdCard.IS_EMPTY;
			return this.error;
		}

		if (!this.isLenCorrect()) {
			this.error = IdCard.LEN_ERROR;
			return this.error;
		}
		...
		if (!this.isDateCorrect()) {
			this.error = IdCard.DATE_ERROR;
			return this.error;
		}

		if (this.is18()) {
			if (!this.getCheck().equals(this.getCheckBit())) {
				this.error = IdCard.CHECK_BIT_ERROR;
				return this.error;
			}
		}
