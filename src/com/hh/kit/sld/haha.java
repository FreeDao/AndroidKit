 ...
 * <p>
 * Population of the tabs to display is done through {@link Tab} instances. You
 * create tabs via {@link #newTab()}. From there you can change the tab's label
 * or icon via {@link Tab#setText(int)} and {@link Tab#setIcon(int)}
 * respectively. To display the tab, you need to add it to the layout via one of
 * the {@link #addTab(Tab)} methods. For example:
 * 
 * <pre>
 * TabLayout tabLayout = ...;
 ...
 * {@link #setOnTabSelectedListener(OnTabSelectedListener)} to be notified when
 * any tab's selection state has been changed.
 * <p>
 * If you're using a {@link android.support.v4.view.ViewPager} together with
 * this layout, you can use {@link #addTabsFromPagerAdapter(PagerAdapter)} which
 * will populate the tabs using the {@link PagerAdapter}'s page titles. You
 * should also use a {@link ViewPager.OnPageChangeListener} to forward the
 * scroll and selection changes to this layout. You can use the one returned
 * {@link #createOnPageChangeListener()} for easy implementation:
 * 
 * <pre>
 * ViewPager viewPager = ...;
 * TabLayout tabLayout = ...;
 ...
 *
 * @see <a
 *      href="http://www.google.com/design/spec/components/tabs.html">Tabs</a>
 */
public class haha extends HorizontalScrollView {
	 ...
	 * contain longer tab labels and a larger number of tabs. They are best used
	 * for browsing contexts in touch interfaces when users don’t need to
	...
	/**
	 * Fixed tabs display all tabs concurrently and are best used with content
	 * that benefits from quick pivots between tabs. The maximum number of tabs
	 * is limited by the view’s width. Fixed tabs have equal width, based on the
	 * widest tab label.
	 *
	 * @attr android.support.design.R.attr.tabMode
	 * @see #setTabMode(int)
	...
	/**
	 * Gravity used to fill the {@link haha} as much as possible. This option
	 * only takes effect when used with {@link #MODE_FIXED}.
	 *
	...
	/**
	 * Gravity used to lay out the tabs in the center of the {@link haha}.
	 *
	 * @attr android.support.design.R.attr.tabGravity
	 * @see #setTabGravity(int)
	 * @see #getTabGravity()
	...
	/**
	 * Set the scroll position of the tabs. This is useful for when the tabs are
	 * being displayed as part of a scrolling container such as
	 * {@link ViewPager}.
	 * <p>
	 * Calling this method does not update the selected tab, it is only used for
	 * drawing purposes.
	 */
	...
	/**
	 * Add new {@link Tab}s populated from a {@link PagerAdapter}. Each tab will
	 * have it's text set to the value returned from
	 * {@link PagerAdapter#getPageTitle(int)}.
	 *
	 * @param adapter
	 *            the adapter to populate from
	 */
	...
	/**
	 * Add a tab to this layout. The tab will be inserted at
	...
	/**
	 * Set the
	 * {@link android.support.design.widget.TabLayout.OnTabSelectedListener}
	...
	/**
	 * Remove a tab from the layout. If the removed tab was selected it will be
	...
	/**
	 * Set the behavior mode for the Tabs in this layout. The valid input
	 * options are:
	 * <ul>
	 * <li>{@link #MODE_FIXED}: Fixed tabs display all tabs concurrently and are
	 * best used with content that benefits from quick pivots between tabs.</li>
	 * <li>{@link #MODE_SCROLLABLE}: Scrollable tabs display a subset of tabs at
	 * any given moment, and can contain longer tab labels and a larger number
	 ...
	 * commonly used with a {@link ViewPager}.</li>
	 * </ul>
	 *
	 * @param mode
	...
	/**
	 * Returns the current mode used by this {@link haha}.
	 *
	 * @see #setTabMode(int)
	 */
	@Mode
	...
	public static final class Tab {
		 ...
		 *
		 * @param resId
		 *            A resource ID referring to the icon that should be
		 *            displayed
		 * @return The current instance for call chaining
		 */
	...
	class TabView extends LinearLayout implements OnLongClickListener {
		...
		public TabView(Context context, Tab tab) {
			...
			if (mTabBackgroundResId != 0) {
				setBackgroundDrawable(TintManager.getDrawable(context,
						mTabBackgroundResId));
			}
			...
			if (mMode == MODE_FIXED && mTabGravity == GRAVITY_CENTER) {
				...
				if (largestTabWidth * count <= getMeasuredWidth() - gutter * 2) {
					...
					for (int i = 0; i < count; i++) {
						final View child = getChildAt(i);
						final LinearLayout.LayoutParams lp = (LayoutParams) child
								.getLayoutParams();
	...
	/**
	 * Linear interpolation between {@code startValue} and {@code endValue} by
	 * the fraction {@code fraction}.
	 */
